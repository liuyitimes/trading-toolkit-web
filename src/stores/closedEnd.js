...
function normalizeFundItem(raw) {
  ...
  const yearsToMaturity =
    maturityDate && !isNaN(maturity)
      ? Math.max(0, (maturity - today) / (365.25 * 24 * 3600 * 1000))
      : 0
  const annualizedDiscount =
    yearsToMaturity > 0 ? discount / yearsToMaturity : 0
      ...
      discount >= 15
        ...
        : discount >= 5
          ...
          : discount < 0
            ...
            : '',
    annualizedDiscount:
      yearsToMaturity > 0 ? annualizedDiscount.toFixed(2) + '%' : '--',
    annualizedRaw: annualizedDiscount,
    maturityDate,
    yearsToMaturity: yearsToMaturity
      ? yearsToMaturity.toFixed(2) + ' 年'
      : '--',
...
function computeSummary(list) {
  ...
  const nearMaturityCount = list.filter(
    (i) => i.yearsRaw > 0 && i.yearsRaw <= 1
  ).length
  return {
    count: list.length,
...
export const useClosedEndStore = defineStore('closedEnd', () => {
  ...
  async function loadAll({ refresh = false } = {}) {
    ...
    try {
      let raw = []
      try {
        const data = await closedEndApi.list(refresh ? { refresh: true } : {})
