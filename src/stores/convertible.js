...
const PLACEMENT_PREMIUM_OPTIONS = [30, 40, 50, 60, 70, 80, 90, 100]
const DEFAULT_PLACEMENT_PREMIUM_RATE = 30
const PLACEMENT_PREMIUM_STORAGE_KEY = 'convertiblePlacementPremiumRate'

function isPlacementPremiumRate(value) {
  return PLACEMENT_PREMIUM_OPTIONS.includes(Number(value))
}

function readPlacementPremiumRate() {
  if (typeof window === 'undefined') return DEFAULT_PLACEMENT_PREMIUM_RATE
  const saved = Number(
    window.localStorage.getItem(PLACEMENT_PREMIUM_STORAGE_KEY)
  )
  return isPlacementPremiumRate(saved) ? saved : DEFAULT_PLACEMENT_PREMIUM_RATE
}
...
function derivePlacementItem(item, premiumRate) {
  ...
  const detail = {
    ...
    safetyPad: metrics.safetyPad == null ? '暂无' : safetyPad,
...
function normalizePendingItem(item) {
  ...
  // Retain API-provided derived values for traceability. The Web scenario
  // values are applied later by derivePlacementItem().
  const apiSafetyPad = item.safety_pad > 0 ? item.safety_pad : null
  const apiExpectedProfit =
    item.expected_profit > 0 ? item.expected_profit : null
  ...
  return {
    placementKey: bondCode || stockCode,
    stockName,
    stockCode,
    exchange,
...
export const useConvertibleStore = defineStore('convertible', () => {
  ...
  function setPlacementPremiumRate(value) {
    ...
    placementPremiumRate.value = isPlacementPremiumRate(next)
      ? next
      : DEFAULT_PLACEMENT_PREMIUM_RATE
    if (typeof window !== 'undefined') {
      window.localStorage.setItem(
        PLACEMENT_PREMIUM_STORAGE_KEY,
        String(placementPremiumRate.value)
      )
    }
  }

  function resetPlacementPremiumRate() {
    setPlacementPremiumRate(DEFAULT_PLACEMENT_PREMIUM_RATE)
  }
  ...
  return {
    ...
    placementPremiumOptions: PLACEMENT_PREMIUM_OPTIONS,
