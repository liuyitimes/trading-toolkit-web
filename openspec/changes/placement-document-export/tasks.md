...
- [ ] 1.1 确认同名 `placement-document-export` 服务端变更通过现有待发配债接口提供可选的逐候选标的发行人证据和公告元数据。依赖服务仓库配套变更落地。
- [x] 1.2 扩展待发项规范化以保留可选来源信息，同时保持旧数组和 `{ items, meta }` 响应兼容。验证：浏览器用例覆盖增强和旧式响应。
...
- [x] 2.3 渲染固定规划观察提示、新鲜度、核验、需复核、来源和不可用状态，不进行推断。验证：聚焦文档与旧响应浏览器用例。
...
- [x] 4.1 增加 Markdown 内容、文件名、当前假设、不可用来源以及未核验或需复核候选标的的聚焦自动化覆盖。验证：`node tests\placement-export.mjs`。
