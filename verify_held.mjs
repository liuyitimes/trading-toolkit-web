...
const data = await page.evaluate(() => {
  ...
  const subBtns = Array.from(document.querySelectorAll('.sub-btn')).map((b) => b.textContent?.replace(/\s+/g, ' ').trim())
  return {
    hasHeldCard: !!heldCard,
    heldCardTitle: heldCard?.querySelector('.held-card-title')?.textContent?.replace(/\s+/g, ' ').trim(),
    heldRowCount: heldRows.length,
    heldRows: Array.from(heldRows).map((r) => r.textContent?.replace(/\s+/g, ' ').trim().slice(0, 100)),
    placementRowCount: placementRows.length,
    placementRows: Array.from(placementRows).map((r) => r.textContent?.replace(/\s+/g, ' ').trim().slice(0, 80)),
    subBtns
  }
})
console.log(JSON.stringify(data, null, 2))

await page.screenshot({ path: 'test_screenshots/held_card.png', fullPage: false })
console.log('\n截图: test_screenshots/held_card.png')
